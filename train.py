...
class Trainer(object):
    ...
    def train_step(self, image, imus, gts, intrinsics, inv_intrinsics) -> tf.Tensor:
        with tf.GradientTape() as tape:
            input_image = tf.stack(image[1:4], axis=0)

            disp = self.disp_net(input_image, training=True)
            ...
            visual_feature = self.visual_net(image, training=True)
            ...
            out_w_pose_avg = tf.stack(out_w_pose_avg, dim=1)  # B, 4, 6

            loss_photo, loss_3d = 0, 0

            # for j in range(3):
            #         pose_j = out2posew(out_w_pose[j]) # (4,2,6) -> (4,2,3,4)
        ...
        # Update metric
        self.train_r_rmse.update_state(gts[:, :, :3], poses[:, :, :3])
        self.train_t_rmse.update_state(gts[:, :, 3:], poses[:, :, 3:])
